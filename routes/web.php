...
Route::get('admin/reset-password/{token}', [App\Http\Controllers\Admin\Auth\ResetPasswordController::class, 'showResetForm'])
    ->name('password.reset'); // 👈 এখানে Laravel ডিফল্ট নামটা ব্যবহার করলাম
...
Route::prefix('admin')
    ->middleware(['auth:admin', 'lock', 'check_refer'])
    ->name('admin.')
    ->group(function () {
...
Route::get('admin/manual-fraud-check', [App\Http\Controllers\Admin\OrderController::class, 'manualFraudCheckPage'])->name('manualFraud.page');
Route::post('admin/manual-fraud-check', [App\Http\Controllers\Admin\OrderController::class, 'manualFraudCheck'])->name('manualFraud.check');
...
// Admin custom SMS
Route::get('/admin/sms/custom-send', [App\Http\Controllers\Admin\ApiIntegrationController::class, 'sms_custom_send_page'])->name('admin.sms.custom.page');
Route::post('/admin/sms/custom-send', [App\Http\Controllers\Admin\ApiIntegrationController::class, 'sms_custom_send'])->name('admin.sms.custom.send');
...
Route::group(['prefix'=>'customer','namespace'=>'Frontend', 'middleware' => ['ipcheck','check_refer']], function() {
    ...
    Route::get('/invoice/download/{id}', [CustomerController::class, 'invoice_download'])->name('customer.invoice_download');
...
Route::post('coupon/update/{id}', [CouponController::class, 'update'])->name('admin.coupons.update');

Route::post('coupon/destroy/{id}', [CouponController::class, 'destroy'])
...
Route::get('reports/profit-loss',   [ReportController::class, 'profitLoss'])->name('admin.reports.profit_loss');
    ...
    // Supplier Routes
    Route::get('suppliers/manage', [SupplierController::class, 'index'])->name('suppliers.index');
